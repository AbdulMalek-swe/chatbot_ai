...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const extractCoordinatesForMessage = (msg: ChatMessage) => {
    ...
    if (msg.map_data?.pois_targeted) {
      msg.map_data.pois_targeted.forEach((p: any) => {
        coords.push({
          lat: p.lat,
          lng: p.lng,
          name: p.label || p.name || `POI (${p.radius_km}km)`,
        });
      });
    }

    const poiData =
      msg.poi_data || msg.campaign_plan?.targetable_poi_coordinates;
    ...
    if (coords.length === 0 && msg.content) {
      const latLngRegex =
        /(?:lat|latitude)[:\s,]*(-?\d+\.\d+)[:\s,]*?(?:lng|longitude)[:\s]*(-?\d+\.\d+)|(?:\()(-?\d+\.\d+)[:\s,]*(-?\d+\.\d+)(?:\))/gi;
      let match;
  ...
  const getContext = () => {
    ...
    return {
      ...
      center: [businessLat, businessLng] as [number, number],
    };
  };
        ...
        {isFirstAI && (
          <div className="shrink-0 flex items-center justify-center transition-all self-start">
            <img
              src="/logo.png"
              alt="Logo"
              className="w-8 h-8 transition-transform group-hover:scale-110 duration-500 rounded-full"
            />
          </div>
        )}
