...
export const MessageBlockUI: React.FC<MessageBlockUIProps> = ({ block }) => {
  ...
  const mainContentPart = (
    <div
      className={`w-full ${isAI ? "text-md leading-relaxed font-medium tracking-tight text-foreground/90" : "font-inter font-normal text-md leading-6 tracking-[0px] text-foreground"}`}
    >
      {block.status === "streaming" && !block.content ? (
        ...
        <div className="whitespace-pre-wrap text-left">{block.content}</div>
      )}
    </div>
  );
