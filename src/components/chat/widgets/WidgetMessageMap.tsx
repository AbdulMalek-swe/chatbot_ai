...
const MapUpdater: React.FC<{ coords: L.LatLngExpression[]; zoom: number }> = ({
  coords,
  zoom,
}) => {
  const map = useMap();
    ...
    <React.Fragment>
      <Circle
        ...
        pathOptions={{
          ...
          className:
            'cursor-pointer hover:fill-opacity-30 transition-all duration-300',
        }}
      />
      <Marker
        position={[lat, lng]}
...
const MessageMap: React.FC<MessageMapProps> = ({
  coordinates = [],
  mapData,
  zoom = 12,
}) => {
        ...
        <div className="absolute top-4 left-10 z-1000 flex flex-col gap-2">
          {mapData ? (
            <div className="bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-slate-200 text-[10px] font-bold text-slate-700 shadow-lg">
              ...
              {(mapData.total_count || 0).toLocaleString()} Device Signals
              Identified
            </div>
          ) : (
            coordinates.slice(0, 2).map((coord, i) => (
              <div
                key={i}
                className="flex items-center gap-2 bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-slate-200 text-[10px] font-bold text-slate-700 shadow-lg"
              >
                <MapPin size={12} className="text-primary-600" />
                <span className="truncate max-w-[120px]">
                  {coord.name || coord.label || `Target ${i + 1}`}
                </span>
              </div>
            ))
          )}
                ...
                >
                  <Popup>
                    <div className="text-[9px] font-mono">
                      Device ID: {maid.maid?.slice(0, 8) || 'Unknown'}...
                    </div>
                  </Popup>
                </CircleMarker>
              );
        ...
        <div className="bg-slate-50 p-4 flex items-center justify-between border-t border-slate-100 relative z-10">
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-[12px] font-medium text-slate-600 italic">
              {mapData
                ? `Visualizing ${mapData.sample_maids?.length || 0} representative device signals across hubs.`
                : `${pointsToFit.length} coordinates mapped.`}
            </span>
          </div>
